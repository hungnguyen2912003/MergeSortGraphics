...
void readfile(int a[MAX], int &n, int &flag) 
{
    ...
    if(n < 2 || n > 10)
    {
        printf("So luong phan tu trong du lieu trong FILE khong hop le! So luong phai tu 2 den 10.\n");
        //flag = 3 => so luong phan tu sai
		flag = 3;
	}
	else
	{
	    ...
	    for(int i = 0; i < n; i++) 
		{
	        ...
	        if (a[i] < 0 || a[i] > 9) 
			{
	            ...
	            printf("Du lieu loi tai vi tri thu %d = %d. Vui long kiem tra va sua FILE.\n", i + 1, a[i]);
	            // flag = 2 => gia tri phan tu sai
				flag = 2;
	            break;
	        }
	    }		
...
void merge_giamdan(int a[MAX], int left, int mid, int right) 
{
    ...
    // Sao chep mang temp v?o mang goc
    for (int i = 0; i < k; i++) 
	{
        a[left + i] = temp[i];
	...
	///Cac su kien mouse click
		if(ismouseclick(WM_LBUTTONDOWN))
		{
			...
			if(x_mouse > 1380 && x_mouse < 1680 && y_mouse > 10 && y_mouse < 80)
			{
				...
				{
				    settextstyle(0, 0, 3);
				    outtextxy(215, 400, "Loi file do so luong phan tu khong hop le");
					outtextxy(350, 440, "Vui long kiem tra va sua FILE");					
				}
			}
